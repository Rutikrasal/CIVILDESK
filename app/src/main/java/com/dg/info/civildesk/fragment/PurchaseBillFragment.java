...
public class PurchaseBillFragment extends BaseFragment {
    private Context context;
    ProgressDialog progressDialog;
    public static int REQUEST_CODE = 102;
    String userid;
    RecyclerView rv_purchasebill_list;
    PurchaseBillAdapter adapter;
    ...
    int msMonth ;
    int msDay;
    public PurchaseBillFragment() {
        // Required empty public constructor
    }

     public static PurchaseBillFragment newInstance() {
        PurchaseBillFragment fragment = new PurchaseBillFragment();
        return fragment;
    }
    ...
    private void getpurchasebilldata() {
        progressDialog.show();
        progressDialog.setMessage("Please Wait!!..");
        userid = localData.getSignIn().getUserId();
        Log.d(TAG, "getpurchasebilldata: userid"+userid);
        Call<ShowPurchasebillResponse> call = RetrofitClient.getRetrofitClient().showpurchasebill(userid);
        call.enqueue(new Callback<ShowPurchasebillResponse>() {
            @Override
            public void onResponse(Call<ShowPurchasebillResponse> call, Response<ShowPurchasebillResponse> response) {
                ...
                if (response.body() != null ) {
                    if (response.body().getResponseCode().matches("0")) {
                        ...
                        if(response.body().getData().size() != 0) {
                            ...
                            adapter=new PurchaseBillAdapter(context,response.body().getData());
            ...
            @Override
            public void onFailure(Call<ShowPurchasebillResponse> call, Throwable t) {
                progressDialog.dismiss();
    ...
    private void additiontotal(ArrayList<String> total) {
        ...
        for (int i= 0;i<total.size();i++)
        {
            ...
            tv_amount.setText(String.valueOf(sumnettotal));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.action_add:
                ...
                Intent lr=new Intent(context, AddPurchaseBillActivity.class);
                lr.putExtra("frag","1");
